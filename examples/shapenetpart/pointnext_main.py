...
def main(gpu, cfg):
   ...
   # 加载自监督参数
    try:
        selfprepath = r"/data_1/home/whs/Pointnext/examples/shapenetpart/log/shapenetpart/unsupervised_pointnext/pointnext_4/checkpoint/_ckpt_latest.pth"
        # selfprepath = r"/root/autodl-tmp/Pointnext/examples/shapenetpart/log/shapenetpart/unsupervised_pointnext/checkpoint/_ckpt_latest.pth"
        current_weights = model.state_dict()  # 参看当前模型的参数模型
        pretrained = torch.load(selfprepath)
        ...
        # cfg.start_epoch = pretrained['epoch']  # 加载当前的预训练epoch
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        if cfg.rank == 0:
            save_checkpoint(cfg, model, epoch, optimizer, scheduler,
                            additioanl_dict={'ins_miou': best_ins_miou, 'cls_miou': cls_miou_when_best}, is_best=is_best)

        # 保存为Excel文件
        model_name = cfg.cfg_basename.split('_')[0]
        # save_path = 'log/pictures/' + model_name  # 指定
        save_path = "log/pictures/pointnext/wheat_pointnext_super_40/"
    ...
    if cfg.get('num_votes', 0) > 0:
        ...
        with np.printoptions(precision=2, suppress=True):
            logging.info(f'---Voting---\nBest Epoch {best_epoch},'f'Voting Instance mIoU {test_ins_miou:.2f}, 'f'Voting Class mIoU {test_cls_miou:.2f}, 'f'\n Voting Class mIoUs {test_cls_mious}')
...
def testmetric(model, val_loader, cfg, num_votes=0, data_transform=None):
    ...
    for idx, data in pbar:
        ...
        leafarr = []
        disarr = []
        predictedpath = r"/data/home/whs/Pointnext/examples/shapenetpart/log/visual/pointnext/wheat_pointnext_super_40/"  # 保存可视化预测结果的txt的路径
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', type=str, default="../../cfgs/shapenetpart/pointnext_default.yaml", help='config file')    # pointnet2_default.yaml   pointnext_default.yaml
