...
from openpoints.models.Adaptive_Graph_Conv.model import Net
from openpoints.models.pointtransformer.model import PointTransformerSeg
from openpoints.models.dgcnn.model import DGCNN_partseg
from openpoints.models.pointnet.pointnet_part_seg import pointnet_part
...
def main(gpu, cfg):
   ...
   # print(type(cfg.dataset))
    ...
    # build dataset
    val_loader = build_dataloader_from_cfg(cfg.batch_size, cfg.dataset, cfg.dataloader,
                                           datatransforms_cfg=cfg.datatransforms, split='val',
                                           distributed=cfg.distributed)
    test_loader = build_dataloader_from_cfg(cfg.batch_size, cfg.dataset, cfg.dataloader,
                                            datatransforms_cfg=cfg.datatransforms, split='test',
                                            distributed=cfg.distributed)
    ...
    device = torch.device('cuda:0')  # model = build_model_from_cfg(cfg.model).cuda()
    # model = Net().to(device)
    model = DGCNN_partseg().to(device)
    # model = PointTransformerSeg().to(device)
    # model = pointnet_part().to(device)
    # # model = pointnet2().to(device)
    # model = Point_Transformer_partseg().to(device)
    logging.info(model)
    ...
    '''方法一'''
    try:
        selfprepath = r"/data_1/home/whs/Pointnext/examples/shapenetpart/log/shapenetpart/unsupervised_dgcnn/checkpoint/_ckpt_latest.pth"
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        if cfg.rank == 0:
            save_checkpoint(cfg, model, epoch, optimizer, scheduler, additioanl_dict={'ins_miou': best_ins_miou, 'cls_miou': cls_miou_when_best}, is_best=is_best)

        # 保存为Excel文件
        print("Start saving data")
        model_name = cfg.cfg_basename.split('_')[0]
        # save_path = 'log/pictures/' + model_name  # 指定
        save_path = "log/pictures/dgcnn/wheat_dgcnn_super_40/"
        # save_path = 'log/pictures/wheat_dgcnn_unsuper_100/'  # 指定
...
def train_one_epoch(model, train_loader, criterion, segcriterion, optimizer, scheduler, epoch, cfg):
    ...
    for idx, data in pbar:
        ...
        # 在第二个维度上进行拼接
        # data_all = np.concatenate((data['pos'], data['x']), axis=1)   # 使用numpy的方式进行拼接，只能在CPU上使用
        ...
        logits = model(data_all,label_one_hot)    # data['pos']  pct,dgcnn,pointnet
        # logits = model(data_all)  # [8,128,4096]   # pointtransformer
    ...
    for idx, data in pbar:
      ...
      #  data['x'] = get_features_by_keys(data, cfg.feature_keys)
        ...
        for v in range(num_votes+1):
            set_random_seed(v)
            if v > 0:
                data_all = data_transform(data_all)
            logits +=  model(data_all, label_one_hot)  # data['pos']  pct,dgcnn,pointnet
            # logits += model(data_all)  # [8,128,4096]   # pointtransformer
        ...
        #     part_seg_refinement(preds, data_all, data['cls'], cfg.cls2parts, cfg.get('refine_n', 10))
        if cfg.criterion_args.NAME != 'MultiShapeCrossEntropy':
...
def testmetric(model, val_loader, cfg, num_votes=0, data_transform=None):
    ...
    for idx, data in pbar:
       ...
       # data['x'] = get_features_by_keys(data, cfg.feature_keys)
        ...
        label_one_hot = label_one_hot.cuda()
        logits = 0
        data_1 = data['pos'].permute(0, 2, 1)
        for v in range(num_votes+1):
            set_random_seed(v)
            if v > 0:
                data['pos'] = data_transform(data_1).permute(0, 2, 1)
            logits += model(data_all, label_one_hot)     #data['pos']   pct,dgcnn,pointnet
            # logits += model(data_all)  # [8,128,4096]   # pointtransformer
        logits /= (num_votes + 1)
        preds = logits.max(dim=1)[1]   #preds即为所求类别
        ...
        leafarr = []
        disarr = []
        predictedpath = "/data/home/whs/Pointnext/examples/shapenetpart/log/visual/dgcnn/wheat_dgcnn_super_40/"  # 保存可视化预测结果的txt的路径
...
if __name__ == "__main__":
    ...
    # dgcnn.yaml  # pointnet_default.yaml
