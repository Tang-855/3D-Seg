...
# from models.DGCNN.model import DGCNN_partseg
...
def main(gpu, cfg):
   ...
   # print(type(cfg.dataset))
    ...
    train_loader = build_dataloader_from_cfg(cfg.batch_size, cfg.dataset, cfg.dataloader,
                                             datatransforms_cfg=cfg.datatransforms, split='train',
   ...
   # if cfg.model.get('decoder_args', None):
    ...
    '''方法一'''
    try:
        selfprepath = r"/data_1/home/whs/Pointnext/examples/shapenetpart/log/shapenetpart/unsupervised_pointnet2/checkpoint/_ckpt_latest.pth"
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        if epoch % cfg.val_freq == 0:     # 验证
            ...
            if val_ins_miou > best_ins_miou:
                ...
                with np.printoptions(precision=2, suppress=True):
                    logging.info(f'Find a better ckpt @E{epoch}, val_ins_miou {best_ins_miou:.2f} val_cls_miou {cls_miou_when_best:.2f}, 'f'\ncls_mious: {cls_mious_when_best}')
        ...
        save_path = "log/pictures/pointnet2/wheat_pointnet2_super_40/"
    ...
    if cfg.get('num_votes', 0) > 0:
        # pretrained_path = r"/data/home/whs/Pointnext/examples/shapenetpart/log/shapenetpart/pointnet2_wheat_90/checkpoint/ckpt_best.pth"
        ...
        # test_ins_miou, test_cls_miou, test_cls_mious  = validate_fn(model, test_loader, cfg, num_votes=cfg.get('num_votes', 0),data_transform=voting_transform)
        with np.printoptions(precision=2, suppress=True):
...
def train_one_epoch(model, train_loader, criterion, segcriterion, optimizer, scheduler, epoch, cfg):
    ...
    for idx, data in pbar:
        ...
        #print(data['x'].shape)
        #print(data['cls'])
        logits = model(data_all,label_one_hot)    # 直接按键值查找data，包括xyz和法向量
        #print(logits.shape)
...
def testmetric(model, val_loader, cfg, num_votes=0, data_transform=None):
    ...
    for idx, data in pbar:
        ...
        leafarr = []
        disarr = []
        predictedpath = "/data/home/whs/Pointnext/examples/shapenetpart/log/visual/pointnet2/wheat_pointnet2_super_40/"  # 保存可视化预测结果的txt的路径
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', type=str, default="../../cfgs/shapenetpart/pointnet2_default.yaml", help='config file')    # pointnet2_default.yaml
