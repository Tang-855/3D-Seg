...
from openpoints.models.pointconv.pointconv_sem_seg import pointconv
...
def main(gpu, cfg):
   ...
   # print(type(cfg.dataset))
    ...
    '''方法一'''
    try:
        selfprepath = r"/data_1/home/whs/Pointnext/examples/shapenetpart/log/shapenetpart/unsupervised/unsupervised_pointconv/checkpoint/_ckpt_latest.pth"
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        save_path = "/data/home/whs/Pointnext/examples/shapenetpart/log/pictures/pointconv/wheat_pointconv_super_5/"
...
def train_one_epoch(model, train_loader, criterion, segcriterion, optimizer, scheduler, epoch, cfg):
    ...
    for idx, data in pbar:
        ...
        logits = model(data_all)    # data['pos']
...
def testmetric(model, val_loader, cfg, num_votes=0, data_transform=None):
    ...
    for idx, data in pbar:
       ...
       # data['x'] = get_features_by_keys(data, cfg.feature_keys)
        ...
        leafarr = []
        disarr = []
        predictedpath = "/data/home/whs/Pointnext/examples/shapenetpart/log/visual/pointconv/wheat_pointconv_super_5/"  # 保存可视化预测结果的txt的路径
...
@torch.no_grad()
def testmetric_copy(model, val_loader, cfg, num_votes=0, data_transform=None):
    model.eval()  # set model to eval mode
    pbar = tqdm(enumerate(val_loader), total=val_loader.__len__())
    cls_mious = torch.zeros(cfg.shape_classes, dtype=torch.float32).cuda(non_blocking=True)
    cls_nums = torch.zeros(cfg.shape_classes, dtype=torch.int32).cuda(non_blocking=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', type=str, default="../../cfgs/shapenetpart/pointconv.yaml", help='config file')
