...
class GroupingOperation(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        # print(idx.shape)
        # features.is_contiguous()   # [8,3,4096], [8,64,4096]   （ [8,128,4096]  ）
        # idx.is_contiguous()        # [8,256,32]
        assert features.is_contiguous()   # [8,3,4096], [8,64,4096]
        assert idx.is_contiguous()        # [8,256,32]
...
class QueryAndGroup_1(nn.Module):
    def __init__(self, radius: float, nsample: int,
                 relative_xyz=True,
                 normalize_dp=False,
                 normalize_by_std=False,
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[
        ...
        features = features.contiguous()
        grouped_features = grouping_operation(features, idx) if features is not None else None
        return grouped_xyz, grouped_features, xyz_trans
...
class KNNGroup(nn.Module):
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[
        ...
        :return:
            new_features: (B, 3 + C, npoint, nsample)
        """
        _, idx = self.knn(support_xyz, query_xyz)    # idx = [8,256,32]   query_xyz = [8,256,3]    support_xyz = [8,4096,3]
        if self.return_only_idx:
            return idx
        idx = idx.int()
        xyz_trans = support_xyz.transpose(1, 2).contiguous()     # xyz_trans = [8,3,4096]
        grouped_xyz = grouping_operation(xyz_trans, idx)         # xyz_trans = [8,3,256,32 ] (B, 3, npoint, nsample)
        if self.relative_xyz:
            grouped_xyz -= query_xyz.transpose(1, 2).unsqueeze(-1)  # relative position   xyz_trans = [8,3,256,32 ] (B, 3, npoint, nsample)
        if self.normalize_dp:
            grouped_xyz /= torch.amax(torch.sqrt(torch.sum(grouped_xyz**2, dim=1)), dim=(1, 2)).view(-1, 1, 1, 1)
        if features is not None:
            grouped_features = grouping_operation(features, idx)    #  [8,128,256,32 ] (B, C, npoint, nsample)
            return grouped_xyz, grouped_features
        else:
            return grouped_xyz, None

class KNNGroup_1(nn.Module):
    def __init__(self, nsample: int,
                 relative_xyz=True,
                 normalize_dp=False,
                 return_only_idx=False,
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[
        ...
        if features is not None:
            ...
            return grouped_xyz, None, xyz_trans
...
# 使用球半径或者KNN实现中心点和邻点的采样
def create_grouper(group_args):
    group_args_copy = copy.deepcopy(group_args)
    method = group_args_copy.pop('NAME', 'ballquery')   # 找到给定的值，如果找到就返回给method，并删除键名。如果没有找到，就返回method给定的值
