...
from ..layers import create_convblock1d,create_convblock2d_withsimam, create_convblock2d, create_act, CHANNEL_MAP, \
    create_grouper, furthest_point_sample, random_sample, three_interpolation, get_aggregation_feautres
from ..simam2 import simam_module2
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
       ...
       # print("mid_channel",mid_channel)
        ...
        #####是否使用simam   create_convblock2d_withsimam加了无参注意力  create_convblock2d没有无参注意力
        create_conv = create_convblock1d if is_head else create_convblock2d_withsimam   #create_convblock2d    #create_convblock2d_withsimam         #create_convblock2d    #create_convblock2d_withsimam  #create_convblock2d
        #use simam or not
        ...
        #######倒瓶颈结构
        if not self.is_head and (self.choosein_channels == 32 or self.choosein_channels == 64):   # 选择到瓶颈的层数
        #if not self.is_head:
            expansion = 2
            ...
            for i in range(len(inveschannels) - 1):
                pwconv.append(create_convblock1d(inveschannels[i], inveschannels[i + 1],
                                                 norm_args=norm_args,
                                                 act_args=act_args if
                                                 (i != len(inveschannels) - 2) and not False else None,
                                                 **conv_args)
                              )
    ...
    def forward(self, pf):
        ...
        if self.is_head:
           # print(f.shape)
            f = self.convs(f)  # (b,c,n)  #第一次MLP提升高维空间
    ...
    def __init__(self, mlp,
        ...
        if not upsample:
            ...
            for i in range(1, len(mlp) - 1):
                ...
                if self.attention:
                    linear1.append(simam_module2())
                    linear1.append(nn.ReLU())
...
class PointNextPartDecoder(nn.Module):
    ...
    def forward(self, p, f, cls_label):
        ...
        #print(f[-len(self.decoder) - 1].shape)
