...
class PointMLP(nn.Module):
    def __init__(self, num_classes=2,points=4096, embed_dim=64, groups=1, res_expansion=1.0,
    ...
    def forward(self, x, norm_plt, cls_label):    # norm_plt = xyz = [8,3,4096]     cls_labelv = [8,1]
        ...
        # here is the decoder
        xyz_list.reverse()   # 将列表中顺序倒过来   [8, 16, 3] --> [8, 64, 3] --> [8, 256, 3] --> [8, 1024, 3] --> [8, 4096, 3]
        x_list.reverse()     # 将列表中顺序倒过来   [8, 1024, 16] --> [8, 512, 64] --> [8, 256, 256] --> [8, 128, 1024] --> [8, 64, 4096]
        x = x_list[0]        #   [8, 1024, 16]
        ...
        '''将cls_label=[8,1]的维度扩大一维变成[8,1,1]，然后利用MLP将特征维度从一维映射到64维'''#
...
def pointMLP(num_classes=2, points=4096, **kwargs) -> PointMLP:
