import numpy as np
from .box_np_ops import points_in_corners,center_to_corner_box3d,points_corners_in_rbbox,rotation_3d_in_axis,corners_nd
...
def assign_box_points_partition(points, gt_boxes, gt_names, num_partition):
    ...
    box_points_mask, box_corners_3d = points_corners_in_rbbox(points, gt_boxes)
    ...
    for i in range(box_points_mask.shape[1]):
        ...
        for j in range(num_partition[gt_names[i]]):
            ...
            partition_points_mask = np.squeeze(points_in_corners(box_points, partition_corners), axis=1)
...
def assign_random_partition(points, gt_boxes, gt_names, num_partition):
    ...
    for i in range(box_points_mask.shape[1]):
        ...
        partition_corners = center_to_corner_box3d(random_partition[:, 0:3], random_partition[:, 3:6], random_partition[:, 6], origin=(0.5, 0.5, 0.5), axis=2)
        ...
        for j in range(num_partition[gt_names[i]]):
            ...
            cur_partition_mask = np.squeeze(points_in_corners(cur_box_points, partition_corners[j:j+1]), axis=1)
            assert len(box_points[partition_points_mask].shape) == 2, 'Wrong box_points[partition_points_mask].shape'
...
class PartAwareAugmentation(object):
    ...
    def remove_empty_gt_boxes(self):
        for i in range(self.num_gt_boxes):
            ...
            if self.random_partition:
                box_points_mask, _ = points_corners_in_rbbox(self.points, self.gt_boxes[i:i+1])
    ...
    def swap_partitions(self, num_swap=1, distance_limit=100, p=1.0, gt_box_idx=None):
        ...
        for i in range(self.num_gt_boxes):
            ...
            target_partition_points[:, :3] = rotation_3d_in_axis(target_partition_points[:, :3], -target_gt_angle, axis=2)
            ...
            target_partition_points[:, :3] = rotation_3d_in_axis(target_partition_points[:, :3],
                                                                            cur_gt_angle, axis=2)
    ...
    def generate_random_noise(self, num_points=30, distance_limit=100, p=0.5, gt_box_idx=None):
        ...
        for i in range(self.num_gt_boxes):
            ...
            for j in range(self.num_partition[self.gt_names[i]]):
                ...
                corners = np.squeeze(corners_nd(dim, origin=(0.5, 0.5, 0.5)), axis=0)
                ...
                generated_points[:, :, :3] = rotation_3d_in_axis(generated_points[:, :, :3], angle, axis=2)
