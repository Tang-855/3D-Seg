...
class PointWOLF(object):  #    # object
    def __init__(self,num_anchors = 4):
        self.num_anchor = 4
        self.sample_type = 'fps'  # ('fps','random')
        self.sigma = 0.5    # Kernel bandwidth

        self.R_range = (-abs(10), abs(10))       # Maximum rotation range of local transformation  局部变换的的旋转范围
        self.S_range = (1., 3)                   # Maximum scailing range of local transformation  局部变换的的缩放范围
        self.T_range = (-abs(0.25), abs(0.25))   # Maximum translation range of local transformation  局部变换的的平移范围
